...
class CLEPCalculusApp {
  ...
  formatExplanationMarkdown(text) {
    if (!text) return '';
    let str = String(text);
    // Replace \texttt{...} with <code class="calc-code">...</code>
  ...
  getQuestionExplanation(q) {
    ...
    if (!hasConcept && !hasSteps && !hasPitfall) {
      const paragraphs = rawText
        ...
        .map(p => p.trim())
        .filter(p => p.length > 0);
      if (paragraphs.length === 0) return `<p>${this.formatExplanationMarkdown(rawText)}</p>`;
      return paragraphs.map(p => {
        const formatted = p.split('\n').map(l => this.formatExplanationMarkdown(l.trim())).join('<br>');
        ...
        solContainer.innerHTML = `
          <div class="inline-solution-card ${isCorrect ? 'correct-banner' : 'incorrect-banner'}" style="margin-top: 1.5rem; padding: 1.25rem 1.5rem; border-radius: var(--radius-md); border: ${isCorrect ? '1px solid rgba(16, 185, 129, 0.4)' : '1px solid rgba(239, 68, 68, 0.4)'}; background: ${isCorrect ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.08)'};">
            ...
            <div class="review-explanation" style="margin-top: 0.85rem; padding-top: 0.85rem; border-top: 1px solid rgba(255, 255, 255, 0.1);">
              <div class="exp-title" style="font-weight: 700; color: var(--accent-primary); margin-bottom: 0.75rem; font-size: 1rem;">💡 Complete Step-by-Step Solution & Explanation:</div>
              <div class="exp-body" style="font-size: 0.95rem; line-height: 1.6;">
    ...
    allQuestions.forEach(q => {
      ...
      card.innerHTML = `
        ...
        <div class="review-explanation">
          <div class="exp-title">Step-by-Step Solution & Explanation:</div>
